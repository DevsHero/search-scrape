    ...
    match scrape_url(&state, url).await {
        Ok(result) => {
            ...
            if result.word_count > 50 {
                ...
                println!("⚠️  LOW QUALITY: Only {} words extracted", result.word_count);
...
async fn boss_2_amazon_product_specs() {
    ...
    match scrape_url(&state, url).await {
        Ok(result) => {
            ...
            let has_recommendations = result.clean_content.to_lowercase().contains("customers who bought") ||
                                     result.clean_content.to_lowercase().contains("frequently bought");
            ...
            println!("\n📄 Content Preview:");
            println!("{}", result.clean_content.chars().take(700).collect::<String>());
        }
        Err(e) => {
            println!("❌ ERROR: {}", e);
        }
    ...
    match scrape_url(&state, url).await {
        Ok(result) => {
            ...
            if result.word_count > 200 {
                println!("✅ PASS: Extracted full article ({} words)", result.word_count);
            } else {
            ...
            println!("\n📄 Content Preview:");
            println!("{}", result.clean_content.chars().take(600).collect::<String>());
        }
        Err(e) => {
            println!("❌ ERROR: {}", e);
        }
...
async fn god_2_tiktok_fingerprint() {
    ...
    match scrape_url(&state, url).await {
        Ok(result) => {
            ...
            let has_block = result.clean_content.contains("captcha") ||
                          result.clean_content.contains("verify");
    ...
    match scrape_url(&state, url).await {
        Ok(result) => {
            ...
            let has_articles = result.clean_content.to_lowercase().contains("article") ||
                             result.clean_content.to_lowercase().contains("search results");
...
async fn benchmark_reddit_subreddit() {
    ...
    match scrape_url(&state, url).await {
        Ok(result) => {
            ...
            let has_threads = result.clean_content.contains("post") ||
                            result.clean_content.contains("upvote");
            
            // Check for excessive sidebar noise
            let sidebar_keywords = ["Rules", "Related", "Moderators", "Community"];
            let noise_count = sidebar_keywords.iter()
